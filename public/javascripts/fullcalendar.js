...
function scroll_to_today(){	
	//logger.debug("function scroll_to_today()"); 
	if($('.fc-view-year')){	$('.fc-view-year').animate({scrollLeft: ($(".fc-today").offset().left - 10)}, 1000);}
} 
(function($) {
...
$.fn.fullCalendar = function(options) {
	...
	delete options.eventSources;
		//logger.debug('options.events.length = ' + options.events.length);
	if (options.events) {
...
;
		...
		function render(inc) {
			//logger.debug("function render(), $('#current_view_mode').val() = " + $('#current_view_mode').val())
			if (elementVisible()) {
		...
		function fetchEvents(callback) {
			//logger.debug("function fetchEvents(callback)")
			events = [];
		...
		function fetchAndRenderEvents() {
			//logger.debug("function fetchAndRenderEvents()" );
		...
		var publicMethods = {
			...
			render: function() {
				//logger.debug("publicMethods.render");
				calcSize();
				sizesDirty();
				eventsDirty();
				render();
			...
			changeView: function() {
					//logger.debug("publicMethods.changeView");
					changeView
				},
			
			getView: function() {
				//logger.debug("publicMethods.getView");
					return view;
				
			},
			
			getDate: function() {
				//logger.debug("publicMethods.getDate");
				return date;
			},
			...
			destroy: function() {
				//logger.debug("publicMethods.destroy");
			...
			prev: function() {
				//logger.debug("publicMethods.prev");
				if (view.name == "year"){addYears(date, -1); render();}
				else{render(-1);}
			},
			
			next: function() {
				//logger.debug("publicMethods.next");
				if (view.name == "year"){addYears(date, 1); render();}
				else{render(1);}
			},
			
			prevYear: function() {
				//logger.debug("publicMethods.prevYear");
				addYears(date, -1);
				render();
			},
			
			nextYear: function() {
				//logger.debug("publicMethods.nextYear");
				addYears(date, 1);
				render();
			},
			
			today: function() {
				//logger.debug("publicMethods.today");
				date = new Date();
				render();
			},
			
			gotoDate: function(year, month, dateNum) {
				//logger.debug("publicMethods.gotoDate");
			...
			incrementDate: function(years, months, days) {
				//logger.debug("publicMethods.incrementDate");
				if (years != undefined) {
					addYears(date, years);
				}
				if (months != undefined) {
			...
			//logger.debug("publicMethods.updateEvent");
			...
			//logger.debug("publicMethods.renderEvent");
				normalizeEvent(event, options);
				if (!event.source) {
					if (stick) {
				...
				//logger.debug("publicMethods.renderEvent event=" + source);
				eventsChanged();

			},
			
			removeEvents: function(filter) {
				//logger.debug("publicMethods.removeEvents");
			...
			clientEvents: function(filter) {
				//logger.debug("publicMethods.clientEvents");
			...
			rerenderEvents: function() {
				//logger.debug("publicMethods.rerenderEvents");
				eventsChanged
			}, // TODO: think of renaming eventsChanged
			...
			addEventSource: function(source) {
				//logger.debug("publicMethods.addEventSource");
				eventSources.push(source);
				fetchEventSource(source, eventsChanged);
			},
		
			removeEventSource: function(source) {
				//logger.debug("publicMethods.removeEventSource");
			...
			refetchEvents: function() {
				//logger.debug("publicMethods.refetchEvents");
...
views.month = function(element, options) {
	$('#current_view_mode').val("month");
	return new Grid(element, options, {
		render: function(date, delta) {
			//logger.debug("month delta = " + delta)
			if (delta) {
...
views.basicWeek = function(element, options) {
	$('#current_view_mode').val("basicWeek");
	return new Grid(element, options, {
		render: function(date, delta) {
			//logger.debug("basicWeek delta = " + delta)
			if (delta) {
...
views.basicDay = function(element, options) {
	$('#current_view_mode').val("basicDay");
	return new Grid(element, options, {
		render: function(date, delta) {
			//logger.debug("basicDay delta = " + delta)
			if (delta) {
				addDays(date, delta);
				if (!options.weekends) {
...
views.year = function(element, options) {
	//logger.debug("VIEWS: views.year")
	$("#current_view_mode").val("year");
	return new Grid(element, options, {
		render: function(date, delta) {
			var start_date = new Date(date.getFullYear(), 0, 1);
			var end_date = new Date(date.getFullYear(), 11, 31, 23, 59, 59);
			var one_day=1000*60*60*24;
			var year_days = Math.round((end_date.getTime()-start_date.getTime())/(one_day));
			//logger.debug("year delta = " + delta)
			if (delta) {
				addDays(date, delta * year_days);
			}
				//alert("start_date, end_date -> " + start_date +", " + end_date)

			var visStart = this.visStart = cloneDate(
					this.start = cloneDate(start_date)
				),
				visEnd = this.visEnd = cloneDate(
					this.end = addDays(cloneDate(start_date), year_days)
				);
				//alert("(visStart / visEnd):   " + visStart + "/" + visEnd)
			this.title = formatDates(
				visStart,
				addDays(cloneDate(visEnd), -1),
				this.option('titleFormat'),
			...
			this.title = formatDate(start_date, this.option('titleFormat'), options);
			...
			$(".fc-view-year").css({"overflow":"auto", "font-size": "0.900em"});
		}
	});
};
...
function Grid(element, options, methods) {
	//logger.debug("function Grid()")
	var view_name = $('#current_view_mode').val();
	var tm, firstDay,
		nwe,            // no weekends (int)
		...
		//logger.debug("function Grid(), view_name = " + view_name)
		if (view_name == "year"){
			var dayContentPositions = new HorizontalPositionCache(function(col) {
					return tbody.find('td:eq(' + col + ') div div');
			})
		}
		else{
			var dayContentPositions = new HorizontalPositionCache(function(dayOfWeek) {
	...
	var view = $.extend(this, viewMethods, methods, {
	...
	function renderGrid(r, c, colFormat, showNumbers) {
		//logger.debug("function renderGrid()")
		rowCnt = r;
		colCnt = c;
	...
	function renderYearGrid(r, c, colFormat, showNumbers) {
		//logger.debug("function renderYearGrid(r, c, colFormat, showNumbers) >" + r +", "+ c +", "+ colFormat +", " + showNumbers )
		rowCnt = r;
		colCnt = c;
		...
		var year = view.start.getFullYear(),
			today = clearTime(new Date()),
			s, i, j, d = cloneDate(view.visStart);
			//logger.debug("renderYearGrid, year = " + year )
			//logger.debug("renderYearGrid, today = " + today )
			//logger.debug("renderYearGrid, cloneDate(view.visStart) = " + cloneDate(view.visStart) )
		
		if (!tbody) { // first time, build all cells from scratch
			//logger.debug("renderYearGrid, 'first time, build all cells from scratch'" )
			var table = $("<table/>").appendTo(element);
			
			s = "<thead><tr>";
			//logger.debug("renderYearGrid, colCnt = " + colCnt )
			for (i=0; i<colCnt; i++) {
				s += "<th class='fc-" +
					dayIDs[d.getDay()] + ' ' + // needs to be first
					tm + '-state-default' +
			...
			for (i=0; i<rowCnt; i++) {
				...
				for (j=0; j<colCnt; j++) {
					s += "<td class='fc-" +
						...
						tm + '-state-default fc-day' + (i*colCnt+j) +
						(j==dit ? ' fc-leftmost' : '') +
						(rowCnt>1 && d.getFullYear() != year ? ' fc-other-year' : '') +
						(+d == +today ?
						' fc-today '+tm+'-state-highlight' :
						' fc-not-today') + "'>" +
						(showNumbers ? "<div class='fc-day-number'>" + d.getDate() + "</div>" : '') +
			...
			//logger.debug("renderYearGrid, 'NOT first time, reuse as many cells as possible'" )
			clearEvents();
			...
			tbody.find('td').each(function() {
				...
				if (rowCnt > 1) {
					if (d.getFullYear() == year) {
						td.removeClass('fc-other-year');
					}else{
						td.addClass('fc-other-year');
					}
				}
				if (+d == +today) {
					td.removeClass('fc-not-today')
...
*/	
	...
	function setWidth(width) {
		//logger.debug("function setWidth(), width = " + width)
		viewWidth = width;
	...
	function renderEvents(events) {
		//logger.debug("function renderEvents()")
	...
	function rerenderEvents(modifiedEventId) {
		//logger.debug("function rerenderEvents()")
		clearEvents();
	...
	function clearEvents() {
		//logger.debug("function clearEvents()")
	...
	function compileSegs(events) {
		//logger.debug("function compileSegs(events), colCnt = " + colCnt)
		...
		//logger.debug("function compileSegs(events), d1 = " + d1)	
		//logger.debug("function compileSegs(events), d2 = " + d2)
	...
	function renderSegs(segs, modifiedEventId) {
		//logger.debug("function renderSegs(), segs -> " +segs)
		_renderDaySegs(
			segs,
			rowCnt,
			view,
...
//logger.debug("function _renderDaySegs() ")
	...
	var view_name = $('#current_view_mode').val();
	if (view_name=="year") { maxLeft = $(".fc-content table").width()}
	//logger.debug("function _renderDaySegs(), maxLeft = " + maxLeft);
	//logger.debug("function _renderDaySegs(), segs.length =" +segs.length);
	
	for (i=0; i<segCnt; i++) {
		seg = segs[i];
		event = seg.event;
		//logger.debug("function _renderDaySegs(), event["+i+"].start =" +event.start);
		...
		if (rtl) {
			...
			if (view_name=="year") {
				...
				var diff_to_start =  seg.start.getTime() - start_date.getTime();
				var days_to_start = Math.floor(diff_to_start / (1000 * 60 * 60 * 24));
				var diff_to_end = seg.end.getTime() - start_date.getTime();
				var days_to_end = Math.floor(diff_to_end / (1000 * 60 * 60 * 24));
				//logger.debug("function _renderDaySegs() days_to_start = " +days_to_start);
				//logger.debug("function _renderDaySegs() days_to_end = " +days_to_end);
				
				left = seg.isStart ? dayContentLeft(days_to_start) : minLeft;
				right = seg.isEnd ? dayContentRight(days_to_end) : maxLeft;
				//logger.debug("function _renderDaySegs() left = " +left);
				//logger.debug("function _renderDaySegs() right = " +right);
			...
			//logger.debug("function _renderDaySegs() event["+i+"], seg.isStart = " +seg.isStart);
			//logger.debug("function _renderDaySegs() event["+i+"], left = " +left);
			//logger.debug("function _renderDaySegs() event["+i+"], right = " +right);
		}
		html +=
...
views.agendaWeek = function(element, options) {
	$('#current_view_mode').val("agendaWeek");
	return new Agenda(element, options, {
		render: function(date, delta) {
			//logger.debug("agendaWeek delta = " + delta)
			if (delta) {
				addDays(date, delta * 7);
			}
			var visStart = this.visStart = cloneDate(
...
views.agendaDay = function(element, options) {
	$('#current_view_mode').val("agendaDay");
	return new Agenda(element, options, {
		render: function(date, delta) {
			//logger.debug("agendaDay delta = " + delta)
			if (delta) {
				addDays(date, delta);
				if (!options.weekends) {
					skipWeekend(date, delta < 0 ? -1 : 1);
...
function Agenda(element, options, methods) {
	...
	function renderAgenda(c, colFormat) {
		colCnt = c;
		//logger.debug("function renderAgenda()")
		// update option-derived variables
		tm = options.theme ? 'ui' : 'fc';
		nwe = options.weekends ? 0 : 1;
		firstDay = options.firstDay;
	...
	function renderEvents(events, modifiedEventId) {
		//logger.debug("function renderEvents()")
		view.reportEvents(cachedEvents = events);
		//logger.debug("renderEvents(), events.length = "+events.length)
	...
	function renderDaySegs(segs, modifiedEventId) {
		//logger.debug("function renderDaySegs()")
		if (options.allDaySlot) {
			_renderDaySegs(
				segs,
				1,
	...
	function renderSlotSegs(segs, modifiedEventId) {
		//logger.debug("function renderSlotSegs()")
...
function HorizontalPositionCache(getElement) {
	//logger.debug("function HorizontalPositionCache()")
	...
	t.left = function(i) {
		//logger.debug("function HorizontalPositionCache(), lefts[i] = " + lefts[i])
		return lefts[i] =
